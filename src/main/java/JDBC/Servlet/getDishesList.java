 ...
 */
public class getDishesList extends HttpServlet{
    @Override
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int pageMax = 0;
        String sql = "SELECT dishesinfo.* FROM dishesinfo WHERE shelvesed=1 LIMIT "+5*(page-1)+",5";
        String sql2 = "SELECT count(*) FROM dishesinfo WHERE shelvesed=1";
        Connection conn = JDBC.getConnection();
        // 尝试将结果结构化为xml文档
        try {
            ResultSet rs = JDBC.select(sql2,conn);
            if(rs.next()){
                int count = rs.getInt(1);  //对总记录数赋值
                pageMax = (int)Math.ceil(count/5.0);
            }
            rs = JDBC.select(sql,conn);
            // 创建XML DOM树
            ...
            while (rs.next()) {
                ...
                // 创建是否推荐子标签
                Element shelvesed = doc.createElement("shelvesed");
                // 设置是否推荐菜品标签文本内容
                shelvesed.setTextContent(rs.getString("shelvesed"));
                // 将是否推荐菜品标签设置为菜品标签的子标签
                dishes.appendChild(shelvesed);
                // 创建菜品价格标签
            ...
            // 设置最大页码数标签的文本内容
            maxPageElement.setTextContent(pageMax + "");
            // 将最大页码数标签设置为根标签的子标签
